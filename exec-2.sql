...
select ENAME, SAL
from scott.emp
where (SAL+ifnull(COMM,0)) > (select max(SAL+ifnull(COMM,0)) from scott.emp where DEPTNO = 30);
# 13. 返回部门号、部门名、部门所在位置及其每个部门的员工总数
select d.DEPTNO, d.DNAME, d.LOC, count(e.ENAME) number
from scott.emp e
       right join scott.dept d
on e.DEPTNO = d.DEPTNO
group by d.DEPTNO;
# 14. 返回员工的姓名、所在部门名及其工资
select emp.ENAME, dept.DNAME, emp.SAL+ifnull(COMM,0)
from scott.emp
       join scott.dept
...
select JOB, min(SAL+ifnull(COMM,0)) minsal
from scott.emp
group by JOB;
# 18. 返回不同部门经理的最低工资
select DEPTNO,job, min(SAL+ifnull(COMM,0))
from scott.emp
where JOB = 'manager'
group by DEPTNO;
# 19. 计算出员工的年薪，并且以年薪排序
...
where e.SAL+ifnull(COMM,0) between s.LOSAL and s.HISAL
 and s.GRADE = 4;